            ...
            {addFoodOpen && (
              <div className="food-add-panel">
                ...
                {state.leftovers.length > 0 && (
                  <div className="food-kitchen">
                    <span className="food-kitchen-label">
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9h18v10a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V9Z"/><path d="M3 9V6a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v3"/></svg>
                      Kitchen
                    </span>
                    <div className="food-kitchen-scroll">
                      {state.leftovers.map((lo) => (
                        <button
                          key={lo.id}
                          className="food-kitchen-chip"
                          onClick={() => {
                            setAddFoodOpen(false);
                            handleQuickAddSelect({
                              type: 'leftover',
                              id: lo.id,
                              name: lo.name,
                              kcal: lo.perServing.kcal,
                              protein: lo.perServing.protein,
                              carbs: lo.perServing.carbs || 0,
                              fat: lo.perServing.fat || 0,
                              remaining: lo.remainingServings,
                              leftover: lo,
                            });
                          }}
                        >
                          <span className="food-kitchen-emoji">{getEmoji(lo.name)}</span>
                          <span className="food-kitchen-name">{lo.name}</span>
                          <span className="food-kitchen-detail">{lo.remainingServings} serving{lo.remainingServings !== 1 ? 's' : ''}</span>
                        </button>
                      ))}
                    </div>
                  </div>
